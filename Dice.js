...
// - 1 เกม สามารถกำหนดจำนวนรอบสูงสุดได้ แต่ละรอบผู้เล่นแต่ละฝ่ายจะมีการโยนลูกเต๋า 1 
// ครั้งต่อฝ่ายผู้เล่น
// - ผู้เล่นแต่ละคนมีการเก็บชื่อผู้เล่น ค่าเลขที่ได้จากการโยนลูกเต๋าในแต่ละรอบ 
// และผลของเกมแต่ละรอบ
...
// - ให้บันทึกผลในแต่ละรอบว่าฝ่ายใดได้ผลใด (Win, Lose, Draw) 
// โดยพิจารณาจากค่าเลขลูกเต๋าที่มีค่ามากกว่าเป็นฝ่ายชนะ
...
function play(gameDice,player1,player2,rounds){
    let p1Win =0; //รอบที่เล่นเเล้วชนะ ยังไม่ชนะก็เป็น0
    let p2Win =0;

    for(let i =0; i< rounds; i++){
        console.log(`round# ${i + 1}`); //เเสดงรอบ
        ...
        return p1Win > p2Win //เช็คตัวเเรก รึป่าว
        ? player1.name //ถ้าใช่ ให้ตอบอันนี้ ถ้าไม่ก็ไปต่อ
        : p2Win > p1Win
        ? player2.name
        : 'Draw'
...
p1 = {name: 'zin', faceValues:[],results:[]};
p2 = {name: 'wombat', faceValues:[],results:[]};

console.log(`${play(dice,p1,p2,1)} Win!`);
